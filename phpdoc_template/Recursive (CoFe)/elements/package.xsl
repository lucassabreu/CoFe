<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="package" mode="sidebar">
        ...
        <li>
            <xsl:if test="$element_count > 0">
                ...
                <a href="{$root}packages/{$link}.html" title="{@name}"><i class="icon-folder-open icon-white"></i> <xsl:value-of select="@name" /></a>
            </xsl:if>

            <xsl:if test="$element_count = 0">
                <span class="empty-package"><i class="icon-folder-close icon-white"></i> <xsl:value-of select="@name" /></span>
            </xsl:if>
    ...
    <xsl:template match="package" mode="contents">
        ...
        >
            <ul class="breadcrumb">
                <li>
                    <a href="{$root}index.html"><i class="icon-folder-open icon-white"></i></a>
    ...
    <xsl:template match="package" mode="sidebar-nav">
        ...
        <div class="btn-group view pull-right" data-toggle="buttons-radio">
          <button class="btn details" title="Show descriptions and method names"><i class="icon-list icon-white"></i></button>
          <button class="btn simple" title="Show only method names"><i class="icon-align-justify icon-white"></i></button>
        </div>

        <ul class="side-nav nav nav-list">
            <li class="nav-header"><i class="icon-map-marker icon-white"></i> Packages</li>
